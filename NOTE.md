...
Your task is to modify the UI for the items in status screen to look like shown in [navbar_example]. More specifically, it should no longer be a card, but a list with dividers between each item. The functionalities should remain the same. The 2 buttons, and the last logged information and the name should remain the same.
